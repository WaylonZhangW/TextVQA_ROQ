...
@registry.register_loss("m4c_decoding_bce_with_obj_loss")
class M4CDecodingBCEWithMaskLoss(nn.Module):
    def __init__(self):
        super().__init__()
        self.one = torch.Tensor([1.])
    ...
    def forward(self, sample_list, model_output):
        ...
        max_element = torch.argmax(F.softmax(scores,-1),-1) # 21*12  ## batch_size * decodeing step
        fake_targets = torch.zeros_like(targets) # 21*12*5050
        for i in range(fake_targets.shape[0]):
            fake_targets[i][range(fake_targets.shape[1]),max_element[i]]=1
        fake_bce = F.binary_cross_entropy_with_logits(
        ...
        obj_region = model_output['obj_region']
        obj_label = model_output['obj_label']
        
        weight = torch.ones(obj_label.size(0),dtype=torch.float32,device=obj_label.device)
        for idx in range(obj_label.size(0)):
            if obj_label[idx].sum() == 0.0:
                weight[idx] = 0.0
        weight = weight.unsqueeze(-1).repeat(1,obj_region.size(1))
        obj_loss = F.binary_cross_entropy_with_logits(obj_region, obj_label, reduction="none",weight=weight)
     
        loss = alpha*loss +  torch.sum(fake_bce)/count + torch.sum(obj_loss) / count
        
        return loss


### 去除anls,alpha = 1
@registry.register_loss("m4c_bce_with_obj_loss_remove_anls")
class M4CDecodingBCEWithMaskLoss(nn.Module):
    def __init__(self):
        super().__init__()
        self.one = torch.Tensor([1.])
    ...
    def forward(self, sample_list, model_output):
        ...
        weight = weight.unsqueeze(-1).repeat(1,obj_region.size(1))
        obj_loss = F.binary_cross_entropy_with_logits(obj_region, obj_label, reduction="none",weight=weight)
        loss = alpha*loss  + torch.sum(obj_loss) / count
        return loss
...
### 去除anls,alpha = 1, beta = 1, 除以count改为除以batch_size
@registry.register_loss("m4c_bce_with_obj_loss_remove_anls_alpha_beta1")
class M4CDecodingBCEWithMaskLoss(nn.Module):
    def __init__(self):
        super().__init__()
        self.one = torch.Tensor([1.])
    ...
    def forward(self, sample_list, model_output):
        ...
        weight = weight.unsqueeze(-1).repeat(1,obj_region.size(1))
        # import pdb; pdb.set_trace()
        bs = obj_label.size(0)  # batch size
        obj_loss = F.binary_cross_entropy_with_logits(obj_region, obj_label, reduction="none",weight=weight)
        # print("vqa loss: ",loss,"obj_loss:",beta * torch.sum(obj_loss) / bs)
        loss = alpha*loss  +  beta * torch.sum(obj_loss) / bs
        return loss
...
@registry.register_loss("m4c_decoding_bce_with_obj_loss_ocr_loss")
class M4CDecodingBCEWithMaskLoss(nn.Module):
    def __init__(self):
        super().__init__()
        self.one = torch.Tensor([1.])
    ...
    def forward(self, sample_list, model_output):
        ...
        obj_loss =  F.binary_cross_entropy(obj_region, obj_label, reduction="none",weight=weight)
        
        ####         
        ocr_region = model_output['ocr_region']
        ocr_label = model_output['ocr_label']
        ocr_weight = torch.ones(ocr_label.size(0),ocr_label.size(1),dtype=torch.float32,device=ocr_label.device)

        for idx in range(ocr_label.size(0)):
            for j in range(ocr_label.size(1)):
                if ocr_label[idx][j] != 0.0:
                    ocr_weight[idx][j] = 0.0
    
        ocr_loss =  F.binary_cross_entropy(ocr_region, ocr_label, reduction="none",weight=ocr_weight)
        
        loss = alpha*loss +  torch.sum(fake_bce)/count + torch.sum(obj_loss) / count + torch.sum(ocr_loss) / count
