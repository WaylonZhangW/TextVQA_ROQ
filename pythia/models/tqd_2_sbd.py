...
import pdb
############ t1: 去掉了simple中的text_embedding_model
########### t2: 在t1的基础上，query ocr 改成和query obj一样的结构,都是2层
########### sbd: 使用sbd ocr
@registry.register_model("tqd_2_sbd")
class TQD(BaseModel):
    ...
    def _init_text_embeddings(self):
        ...
        self.q_to_mmt_in = nn.Linear(self.text_embedding_model.text_out_dim, self.mmt_config.hidden_size)
        self.q_obj_to_mmt_in = nn.Linear(self.text_embedding_model.text_out_dim, self.mmt_config.hidden_size)
        self.q_ocr_obj_to_mmt_in = nn.Linear(self.text_embedding_model.text_out_dim, self.mmt_config.hidden_size)
...
################################################
    ...
    def _forward_ocr_encoding(self, sample_list, fwd_results):
        ...
        ocr_feat = torch.cat(
            [ocr_txt,ocr_fasttext, ocr_phoc, ocr_fc7, ocr_recogcnn],
            # [fwd_results['ocr_txt_emb'],ocr_fasttext, ocr_phoc, ocr_fc7],
            dim=-1
        )
