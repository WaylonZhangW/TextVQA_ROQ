...
class FeatureReader:
    ...
    def _init_reader(self):
        ...
        # read the ocr faster rcnn feature, size : [num,512,7,7]
        elif self.ndim == 4 :
            self.feat_reader = OcrFasterRCNNFeatureReader(self.max_features)
...
class PaddedFasterRCNNFeatureReader:
    ...
    def read(self, image_feat_path):
        ...
        tmp_image_feat[0:image_loc,] = image_feature[0:image_loc,]
...
class OcrFasterRCNNFeatureReader:
    def __init__(self, max_loc):
        '''
        Feature Reader Class for ocr faster rcnn feature
        feature size: (num,512,7,7)
        '''
        self.max_loc = 50
        self.first = True
        self.take_item = False
    ...
    def read(self, image_feat_path):
        ...
        image_loc, n, h, w = image_feature.shape
        tmp_image_feat = np.zeros((self.max_loc, n, h, w), dtype=np.float32)
        image_loc = min(self.max_loc, image_loc)
        tmp_image_feat[0:image_loc,] = image_feature[0:image_loc,]
        image_feature = torch.from_numpy(tmp_image_feat)
