...
def draw_image(entry):
    path = '/public/home/zhangwei1/datasets/data/imdb/microsoft_ocr/microsoftOCR_spatial_val.npy'
    imdb = np.load(path, allow_pickle=True)[1:]
    save_dict = {}
    for item in imdb:
        if int(item["question_id"]) == int(entry["question_id"]):
            save_dict['image_path'] = item['image_path']
            save_dict['question_tokens'] = item['question_tokens']
            break

    entry.update(save_dict)
    return entry
...
class M4CTextVQADataset(TextVQADataset):
    ...
    def load_item(self, idx):
        ...
        current_sample.image_id = sample_info["image_id"]  # type : str
    ...
    def add_sample_details(self, sample_info, sample):
        ...
        ### Get obj text bert tokenizer
        processed_obj = self.obj_text_processor(sample_info['obj_text'])
        ...
        context = self.context_processor({"tokens": ocr_tokens})  # go to the VocabProcessor in processors.py
        sample.context = context["text"]
        sample.context_tokens = context["tokens"]      #### ocr_tokens,padding to 50 with <pad>
        ...
        ### Get ocr text bert tokenizer
        # ocr_str = " ".join(sample.context_tokens)
        processed_ocr = self.ocr_text_processor(ocr_tokens[:100])
        ...
        if 'ocr_normalized_boxes' in sample_info:
            ...
            )["bbox"].coordinates                     # go to BBoxProcessor in processors.py


        # obj_loss
        if ('obj_label' in sample_info):
            if isinstance(sample_info['obj_label'],list):
                sample.obj_label = torch.zeros(sample.obj_bbox_coordinates.size(0),dtype=torch.float32,)
            else:
                sample.obj_label = torch.from_numpy(sample_info['obj_label'])
        else:
            sample.obj_label = torch.zeros(sample.obj_bbox_coordinates.size(0),dtype=torch.float32,)

        # ocr_loss

        if ('ocr_label' in sample_info):
            sample.ocr_label = torch.from_numpy(sample_info['ocr_label'])
        else:
            sample.ocr_label = torch.zeros(sample.context_feature_0.size(0),dtype=torch.float32,)


        #  OCR M matrix
        if ('Distance_Matrix' in sample_info):
            distance_matrix = sample_info['Distance_Matrix']
            sample.distance_matrix = torch.from_numpy(distance_matrix)

        ### TAP question feauture and ocr feature
        # if 'tap_question_feature' in sample_info:
        #     tap_question = sample_info['tap_question_feature']
        #     sample.tap_question = torch.from_numpy(tap_question)

        if 'tap_ocr_feature' in sample_info:
            tap_ocr = sample_info['tap_ocr_feature']
            sample.tap_ocr = torch.from_numpy(tap_ocr)
