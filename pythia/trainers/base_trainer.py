...
class BaseTrainer:
    ...
    def load_model(self):
        ...
        # print('# TQD parameters:', sum(param.numel() for param in self.model.parameters()))
        ...
        if (
            ...
            # zhang: remove parameter: check_reduction=True, find_unused_parameters=True
    ...
    def _load_pretrain(self):
        ...
        self.writer.write("Load others layer of TAP pre-train model!" )

        path = '/public/home/zhangwei1/datasets/TAP/save/pretrained/textvqa_tap_ocrcc_pretrain.ckpt'
        TAP_pretrain = torch.load(path)['model']
        new_dict = {}
        for k, v in TAP_pretrain.items():
            if ('mmt.encoder' in k):
                new_dict[k] = v

        del TAP_pretrain
        model_dict = self.model.state_dict()
        # print(model_dict['mmt.encoder.layer.2.attention.self.key.weight'])
        Our_pretrain = { k:v for k,v in new_dict.items() if k in model_dict }
        # import pdb;pdb.set_trace()
        model_dict.update(Our_pretrain)
        self.model.load_state_dict(model_dict)
        # print(self.model.state_dict()['mmt.encoder.layer.2.attention.self.key.weight'])
    ...
    def config_based_setup(self):
        ...
        torch.cuda.manual_seed(seed)      # new added,
        torch.cuda.manual_seed_all(seed)   # new added, multi-GPU
        np.random.seed(seed)               # new added, numpy module
    ...
    def _run_scheduler(self):
        if self.lr_scheduler is not None:
            ...
            self.lr_scheduler.step() #  zhang: remove the parameter: self.current_iteration
    ...
    def _update_meter(self, report, meter=None, eval_mode=False):
        ...
        with torch.no_grad():
            ...
            batch_size = len(report.question_id)   #### 当前batch中样本的个数，最后一个batch中样本的个数可能小于batch size的大小
            meter.update(meter_update_dict,batch_size)
    ...
    def _logistics(self, report):
        ...
        if should_print is True:
            ...
            extra.update(
                {
                    "lr": "{:.7f}".format(self.optimizer.param_groups[0]["lr"]),
